	...
	uploadData &&
		uploadData.map((item) => {
			let tempObj = {
				...
				isGroup: item.isGroup.toString().toLowerCase() === "yes" || item.isGroup.toString().toLowerCase() === 'true' ? "true" : "false",
				leng: item.leng,
				breath: item.breath,
				height: item.height,
				...
				isTag: item.isTag.toString().toLowerCase() === "yes"|| item.isTag.toString().toLowerCase() === 'true' ? "true" : "false",
				qrCode: item.qrCode,
				isDelete: false,
				isActive: "",
				compStatus: "",
